CREATE DATABASE IF NOT EXISTS QLDangKyHP
...
CREATE TABLE DieuKienHP (
    ...
    CONSTRAINT PK_DieuKienHP
        PRIMARY KEY (MaHP, MaHPTruoc),
    CONSTRAINT FK_DK_HP
        FOREIGN KEY (MaHP)      REFERENCES HocPhan(MaHP),
    CONSTRAINT FK_DK_HPTruoc
        FOREIGN KEY (MaHPTruoc) REFERENCES HocPhan(MaHP),
    CONSTRAINT CK_DK_Loai
        CHECK (LoaiDK IN ('a','b','c'))
);
...
CREATE TABLE ChuongTrinhKhung (
    ...
    CONSTRAINT PK_CTK
        PRIMARY KEY (MaNganh, MaHP),
    CONSTRAINT FK_CTK_Nganh
        FOREIGN KEY (MaNganh) REFERENCES Nganh(MaNganh),
    CONSTRAINT FK_CTK_HP
        FOREIGN KEY (MaHP)    REFERENCES HocPhan(MaHP),
    CONSTRAINT CK_CTK_Loai
        CHECK (LoaiHP IN ('BB','TC')),
    CONSTRAINT CK_CTK_HK
        CHECK (HocKyDeNghi BETWEEN 1 AND 12)
);
...
CREATE TABLE HocKy (
    ...
    CONSTRAINT CK_HK_Status CHECK (TrangThai IN (
        'Chưa mở', 'Đang mở đăng ký', 'Đã đóng đăng ký', 'Đang học', 'Kết thúc'))
);
...
CREATE TABLE LopHocPhan (
    ...
    CONSTRAINT CK_LHP_SiSo      CHECK (SiSoHienTai >= 0 AND SiSoHienTai <= SiSoToiDa),
    CONSTRAINT CK_LHP_Status    CHECK (TrangThai IN ('Đang mở', 'Đã đầy', 'Đã đóng', 'Đã hủy'))
);

CREATE INDEX IX_LHP_HP_HK ON LopHocPhan(MaHP, MaHocKy);
CREATE INDEX IX_LHP_GV    ON LopHocPhan(MaGV, MaHocKy, ThuHoc, TietBatDau);
...
CREATE TABLE SinhVien (
    ...
    CONSTRAINT CK_SV_Status  CHECK (TrangThai IN ('Đang học', 'Bảo lưu', 'Tốt nghiệp', 'Thôi học'))
);
...
CREATE TABLE TaiKhoan (
    ...
    CONSTRAINT CK_TK_VaiTro  CHECK (VaiTro IN ('Admin','GiangVien','SinhVien')),
    -- Chỉ được link 1 trong 2: SV hoặc GV
    CONSTRAINT CK_TK_User    CHECK (
        (MaSV IS NOT NULL AND MaGV IS NULL) OR
        (MaSV IS NULL     AND MaGV IS NOT NULL) OR
        (MaSV IS NULL     AND MaGV IS NULL))  -- Admin không link
);
...
CREATE TABLE DangKyHocPhan (
    ...
    CONSTRAINT CK_DK_Status CHECK (TrangThai IN ('Đã đăng ký', 'Đã hủy', 'Hoàn thành'))
);
...
CREATE TABLE Diem (
    ...
    DiemTK      DECIMAL(4,2) GENERATED ALWAYS AS (
        CASE WHEN DiemQT IS NOT NULL AND DiemThi IS NOT NULL
             THEN CAST(DiemQT * 0.4 + DiemThi * 0.6 AS DECIMAL(4,2))
             ELSE NULL END
    ) STORED,
    
    XepLoai     VARCHAR(1) GENERATED ALWAYS AS (
        CASE
            WHEN DiemQT IS NULL OR DiemThi IS NULL THEN NULL
    ...
    CONSTRAINT CK_Diem_QT    CHECK (DiemQT  IS NULL OR (DiemQT  >= 0 AND DiemQT <= 10)),
    CONSTRAINT CK_Diem_Thi   CHECK (DiemThi IS NULL OR (DiemThi >= 0 AND DiemThi <= 10))
);
...
SELECT
    ...
    hk.TenHocKy,
    d.DiemQT, d.DiemThi, d.DiemTK, d.XepLoai,
    hp.CoTinhGPA,
    lhp.MaLHP
...
WHERE dk.TrangThai = 'Đã đăng ký';

-- View: GPA từng học kỳ
CREATE OR REPLACE VIEW V_GPA_HocKy AS
SELECT
    v.MaSV, v.TenSV, v.TenHocKy,
    SUM(CASE WHEN v.CoTinhGPA=1 AND v.DiemTK IS NOT NULL
             THEN v.SoTinChi ELSE 0 END) AS TongTCTinhGPA,
    ROUND(
        SUM(CASE WHEN v.CoTinhGPA=1 AND v.DiemTK IS NOT NULL
                 THEN v.DiemTK * v.SoTinChi ELSE 0 END) /
        NULLIF(SUM(CASE WHEN v.CoTinhGPA=1 AND v.DiemTK IS NOT NULL
                        THEN v.SoTinChi ELSE 0 END), 0)
    , 2) AS GPA_HocKy
FROM V_BangDiemSinhVien v
GROUP BY v.MaSV, v.TenSV, v.TenHocKy;

-- View: GPA tích lũy
CREATE OR REPLACE VIEW V_GPA_TichLuy AS
SELECT
    sv.MaSV, sv.HoTen,
    SUM(CASE WHEN hp.CoTinhGPA=1 AND d.DiemTK IS NOT NULL
             THEN hp.SoTinChi ELSE 0 END) AS TongTCTichLuy,
    ROUND(
        SUM(CASE WHEN hp.CoTinhGPA=1 AND d.DiemTK IS NOT NULL
                 THEN d.DiemTK * hp.SoTinChi ELSE 0 END) /
        NULLIF(SUM(CASE WHEN hp.CoTinhGPA=1 AND d.DiemTK IS NOT NULL
                        THEN hp.SoTinChi ELSE 0 END), 0)
    , 2) AS GPA_TichLuy
FROM SinhVien sv
LEFT JOIN DangKyHocPhan dk ON sv.MaSV = dk.MaSV AND dk.TrangThai = 'Đã đăng ký'
